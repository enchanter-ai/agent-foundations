...
- **Self-test fixtures.** [`docs/self-test.md`](docs/self-test.md) ships the A/B fixture methodology; **19 of 19 modules** have shipped fixtures as of 2026-05-06 (complete inventory coverage). On **Sonnet 4.6** as subject: 5 of 19 show clear behavioral delta; the other 12 show none — baselines often cite framework terminology unprompted (training contamination). **On Haiku 4.5** as subject (sampled rerun of 5 high-contamination Sonnet-no-delta fixtures): **4 of 5 show clear behavioral delta**. **The contamination hypothesis is supported by data.** The honest claim: *modules act as documentation of behavior on Sonnet and as runtime guidance on Haiku.* See [`docs/self-test.md`](docs/self-test.md) § Haiku-tier rerun for the full reading. [`recipes/stupid-agent-review.md`](recipes/stupid-agent-review.md) ships the runtime architecture; [`tests/runner.py`](tests/runner.py) ships a reference Python implementation (~280 lines, anthropic SDK + stdlib).
