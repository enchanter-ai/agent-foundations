...
- **Self-test fixtures.** [`docs/self-test.md`](docs/self-test.md) ships the A/B fixture methodology; **19 of 19 modules** have shipped fixtures as of 2026-05-06 (complete inventory coverage). Of those 19, **5 show clear behavioral delta** on Sonnet 4.6: `discipline.md` (drift-inviting prompt), `formatting.md` (sandwich anchor), `context.md` (gate duplication), `latency-budgeting.md` (60s threshold + fan-out), `eval-driven-self-improvement.md` (RC-ID regression cases). The other 12 show no measurable delta on this tier — baselines reach the same outcome, frequently citing framework terminology unprompted (training contamination). **This is itself a finding the framework's honest-numbers principle requires us to ship.** Cross-batch hypothesis (now supported by 19 fixtures): *modules prescribing specific structural artifacts discriminate; modules prescribing reasoning practices do not — at least on Sonnet*. See [`docs/self-test.md`](docs/self-test.md) § Cross-batch finding for the full reading and [`recipes/stupid-agent-review.md`](recipes/stupid-agent-review.md) for the runtime.
- **Real-world adoption signal.** Until a downstream project reports on living with the conduct, every module is a hypothesis. Even the 5 with measured Sonnet delta are single-tier proofs; running on Haiku is the recommended next experiment.
