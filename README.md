...
- **Self-test fixtures.** [`docs/self-test.md`](docs/self-test.md) ships the A/B fixture methodology; **8 of 19 modules** have shipped fixtures as of 2026-05-05 (`discipline`, `doubt-engine`, `multi-turn-negotiation`, `verification`, `tool-use`, `formatting`, `refusal-and-recovery`, `failure-modes`). Of those 8, **2 show clear behavioral delta** on Sonnet 4.6 (`discipline.md` under drift-inviting prompts; `formatting.md` on the GPT-5 sandwich rule). The other 6 show form-only delta — multiple baselines cited framework terminology unprompted, suggesting Sonnet has been trained on this material. **This is itself a finding the framework's honest-numbers principle requires us to ship.** See [`docs/self-test.md`](docs/self-test.md) § What the 2026-05-05 batch revealed for the full reading and [`recipes/stupid-agent-review.md`](recipes/stupid-agent-review.md) for the runtime. 11 modules remain operational hypotheses (some untestable on a single-prompt fixture without runtime infrastructure).
